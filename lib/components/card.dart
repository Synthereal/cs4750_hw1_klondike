...
class Card extends PositionComponent
  ...
  final List<Card> attachedCards = <Card>[];
  ...
  static final Paint blackBorderPaint = Paint()
    ...
    ..style = PaintingStyle.stroke
    ..strokeWidth = 10;
  static final Paint blueFilter = Paint()
  ...
  void _renderFront(Canvas canvas) {
    ...
    final Sprite suitSprite = suit.sprite;
  ...
  void onDragStart(DragStartEvent event) {
    ...
    if (pile?.canMoveCard(this, MoveMethod.drag) ?? false) {
      ...
      if (pile is TableauPile) {
        attachedCards.clear();
        final List<Card> extraCards = (pile! as TableauPile).cardsOnTop(this);
        for (final Card card in extraCards) {
  ...
  void onDragUpdate(DragUpdateEvent event) {
    ...
    final Vector2 delta = event.localDelta;
    position.add(delta);
    for (final Card card in attachedCards) {
  ...
  void onDragEnd(DragEndEvent event) {
    ...
    final List<Pile> dropPiles = parent!
    ...
    if (dropPiles.isNotEmpty) {
      if (dropPiles.first.canAcceptCard(this)) {
        ...
        if (dropPiles.first is TableauPile) {
          ...
          final NotifyingVector2 dropPosition =
    ...
    if (attachedCards.isNotEmpty) {
      for (final Card card in attachedCards) {
        final Vector2 offset = card.position - position;
        card.doMove(
...
// if it is face-down and on the Stock Pile, pass the event to that pile.
  ...
  void handleTapUp() {
    ...
    if (pile?.canMoveCard(this, MoveMethod.tap) ?? false) {
      final int suitIndex = suit.value;
  ...
  void doMove(Vector2 to,
    ...
    final double dt = (to - position).length / (speed * size.x);
    assert(dt > 0, 'Distance to move must be > 0');
    add(CardMoveEffect(
        to, EffectController(duration: dt, startDelay: start, curve: curve),
        transitPriority: startPriority, onComplete: () => onComplete?.call()));
  }

  void doMoveAndFlip(Vector2 to,
      {double speed = 10.0,
      double start = 0.0,
      Curve curve = Curves.easeOutQuad,

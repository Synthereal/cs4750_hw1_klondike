...
class TableauPile extends PositionComponent implements Pile {
  ...
  bool canAcceptCard(Card card) {
    if (_cards.isEmpty) {
      ...
      final Card topCard = _cards.last;
  ...
  void dropCards(Card firstCard, [List<Card> attachedCards = const <Card>[]]) {
    final List<Card> cardList = <Card>[firstCard];
    cardList.addAll(attachedCards);
    Vector2 nextPosition = _cards.isEmpty ? position : _cards.last.position;
    int nCardsToMove = cardList.length;
